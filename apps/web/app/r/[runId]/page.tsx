...
export default function RunPage({ params }: { params: { runId: string } }) {
  ...
  useEffect(() => {
    let active = true;
    getResearch(params.runId).then((value) => { if (active) { setResult(value); if (value.status !== "pending") setStage(value.status); } }).catch((cause) => active && setError(cause instanceof Error ? cause.message : "Run not found."));
  ...
  if (error && !result) return <main className="shell"><header className="header"><Link className="wordmark" href="/">DeepScout</Link></header><div className="state"><h1>Research run failed</h1><p className="error">{error}</p><Link className="primary link-button" href="/">Try again</Link></div></main>;
